...
class Diff_Equ:
    ...
    def direction(self, i, j, r):
        ...
        if (t_or_f == 1 and np.linalg.norm(-r[:, i] + self.r_D) + self.radii[i] > np.linalg.norm(
                -point + self.r_D)) or (
                np.min([np.linalg.norm(r[:, i] - wall[0, :]), np.linalg.norm(r[:, i] - wall[1, :])]) < np.linalg.norm(
                2 * self.radii[i]) and np.linalg.norm(-r[:, i] + self.r_D) > np.linalg.norm(-point + self.r_D)):
